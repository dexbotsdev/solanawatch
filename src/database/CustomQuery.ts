...
export const getPremarketingCalls = async (tokenAddress: any) => {
    ...
    const data: ChannelLogs[] = await sequelize.query(
        `SELECT distinct  channelName,callerPostId ,userId,tokenMC ,priceChange24 from ChannelLogs where tokenAddress ='${tokenAddress}' and tokenMC=0   and  channelName not in (
...
export const getKohlsStats = async (tokenAddress: any, tradeSignal: { tokenMC: number; }) => {
    const result = [];
    const data: ChannelLogs[] = await sequelize.query(
        `SELECT  distinct channelName,callerPostId ,userId,tokenMC ,priceChange24 from ChannelLogs where tokenAddress ='${tokenAddress}'  and  channelName not in (
            '${dexscreener_channel}','${soltrending_channel}','${safeguard_channel}','${lpburned_channel}'
        )   `,
        {
            type: QueryTypes.SELECT
